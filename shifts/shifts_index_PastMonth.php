...
$sql = "SELECT DISTINCT a.steam_id, a.discord_name, a.callsign, a.char_name, a.rank, (SELECT SUM(b.duration) FROM public_verified_shifts b WHERE b.steam_id=a.steam_id and b.time_out > '$MonthAgo') duration FROM public_verified_shifts a  where a.callsign is not null order by duration desc LIMIT $obj->start,$limit";
$tData = Query($sql);
